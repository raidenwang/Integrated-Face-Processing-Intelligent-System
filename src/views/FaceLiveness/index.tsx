...
const FaceLivenessDetection = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data } = await axios.post('http://127.0.0.1:6006/face-liveness', formData, {
    ...
    >
      ...
      >
        ...
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          {livenessScore !== null && boundingBox && (
            <div>
              <Paragraph strong>检测结果：</Paragraph>
              <ul>
                <li>
                  <strong>活体分数：</strong> {livenessScore.toFixed(4)}
                </li>
                <li>
                  <strong>人脸边界框：</strong> [{boundingBox.join(', ')}]
                </li>
              </ul>
            </div>
          )}
